...
def train(configs, model, dataset, sampling_rate=0.2, device='cuda'):
    ...
    # Sample training data (rate=sampling_rate)
    sample_indices = torch.randperm(len(coords))[:int(len(coords) * sampling_rate)]
    ...
    for step in process_bar:
        ...
        # For BACON training
        if type(preds) == list:
            loss = [(out[sample_indices] - labels[sample_indices])**2 for out in preds]
            loss = torch.stack(loss).mean()
            preds = preds[-1]
        else:
            #loss = ((preds[sample_indices] - labels[sample_indices]) ** 2).mean()       # MSE loss
            loss = ((preds[sample_indices] - labels[sample_indices]) ** 2).mean()       # MSE loss
            full_loss = ((preds - labels) ** 2).mean()
        ...
        # udpate progress bar
        process_bar.set_description(f"psnr: {psnr_score:.2f}, ssim: {ssim_score*100:.2f}, loss: {loss.item():.4f}, full: {full_loss.item():.4f}")
...
@hydra.main(version_base=None, config_path='config', config_name='sample_image')
def main(configs):
    seed_everything(42)
    ...
    # train
    psnr, ssim = train(configs, model, dataset, sampling_rate=1.0, device=configs.TRAIN_CONFIGS.device)
